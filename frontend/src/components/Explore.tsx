...
const Explore: React.FC<ExploreProps> = ({ name, link }) => {
    ...
    return (
        <div>
            <button
                onClick={handleClick}
                ref={buttonRef}
                className={`border-2 border-white border-opacity-50 text-white bg-black
                   px-3 py-1.5 sm:px-4 sm:py-2 md:px-6 md:py-2 lg:px-8 lg:py-2.5 xl:px-10 xl:py-3
                   rounded-full
                   text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl
                   transition-all duration-200 ease-in-out
                   ${name.toLowerCase() === 'teacher' ? 'hover:border-purple-600' : 'hover:border-yellow-400'}
                   `}
